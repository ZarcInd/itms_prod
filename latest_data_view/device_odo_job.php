...
// Vehicle lookup 
    ...
    $fleetNo = $vi['vehicle_no'];   // no fallback, must exist
    $depo    = $vi['depot'];        // no fallback, must exist
} else {
    // 🚨 Safety net: if somehow missing, log it (should not happen)
    logMsg("⚠️ Vehicle info missing for device $deviceId");
    $fleetNo = '';
    $depo    = '';
}
